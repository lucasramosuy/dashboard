...
export const JournalView: React.FC = () => {
  ...
  const [journal, setJournal] = useState<Partial<PracticeJournal>>({
    content: "",
    subject_id: "",
  });
  ...
  const handleSave = async () => {
    ...
    try {
      const saved = await api.upsertJournal(token, {
        ...journal,
        date: new Date(currentDate), // ✅ Date, no string
      });
      setJournal(saved);
  ...
  return (
    <div
      className="oat-journal-container"
    ...
    >
      ...
      <section className="oat-card">
        <div style={{ marginBottom: "1.5rem" }}>
          ...
          <select
            ...
            onChange={(e) =>
              setJournal({ ...journal, subject_id: e.target.value })
            }
          >
            {subjects.map((s) => (
      ...
      >
        <small style={{ color: "#888" }}>
          Tip: Podés usar **negrita**, *itálica* y listas con guiones.
        </small>
      </div>
    </div>
  );
