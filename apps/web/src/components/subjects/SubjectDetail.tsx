...
export const SubjectDetail: React.FC<Props> = ({ id }) => {
  ...
  const riskVariant =
    info.status === "danger" ? "danger" : info.status === "warning" ? "warning" : "success";
      ...
      <div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6">
          <StatTile
            ...
            tone={riskVariant === "success" ? "default" : riskVariant}
          />
          <StatTile
            label="Faltas"
          ...
          <StatTile
            ...
            value={gradeAvg ?? "—"}
            hint={
              graded.length ? `${graded.length} nota${graded.length > 1 ? "s" : ""}` : "Sin notas"
            }
          />
          <StatTile
            label="Pendientes"
          ...
          >
            <div className="mb-4">
              <ProgressBar
                ...
                tone={
                  info.status === "danger" ? "danger" : info.status === "warning" ? "warning" : "ok"
                }
                label="Faltas usadas"
              />
              <p className="m-0 mt-1.5 text-xs text-theme-text-muted">
                Justificada = media falta. Límite: {info.maxAbsences} faltas (25% de{" "}
            ...
            {sortedAbsences.length === 0 ? (
              ...
              <ul className="list-none p-0 m-0" aria-label="Historial de inasistencias">
                {sortedAbsences.map((a) => (
                  <li
                    key={a.id}
                    className="flex justify-between items-center py-2 border-b border-theme-border last:border-b-0"
                  >
                    <span className="text-sm">
                      {formatDayShort(a.date)}
                      <span className="ml-2 text-xs text-theme-text-muted">
                        {a.calculated_value === 0.5 ? "Justificada · 0,5" : "Falta · 1"}
                      </span>
                    </span>
                    <IconButton
                      label={`Eliminar inasistencia del ${formatDay(a.date)}`}
                      danger
            ...
            {sortedTasks.length === 0 ? (
              ...
              <ul className="list-none p-0 m-0" aria-label="Tareas de la UC">
                {sortedTasks.map((t) => (
                  <li
                    key={t.id}
                    className="flex items-center justify-between gap-3 py-2.5 border-b border-theme-border last:border-b-0"
                  >
                    <a
                      href={url(`/tasks/${t.slug || t.id}`)}
                      className="min-w-0 no-underline text-theme-text"
                    >
                      <span
                        className={`block text-sm font-medium truncate ${t.status === "done" ? "text-theme-text-muted" : ""}`}
                      >
                        {t.title}
                      </span>
                      <span className="block text-xs text-theme-text-muted">
                        {formatDayShort(t.due_date)}
