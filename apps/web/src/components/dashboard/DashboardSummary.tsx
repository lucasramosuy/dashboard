...
export const DashboardSummary: React.FC = () => {
  ...
  const upcoming = [...pending].sort((a, b) => toLocalDay(a.due_date).getTime() - toLocalDay(b.due_date).getTime());
  ...
  const attendance = subjects
    .map((s) => {
      const value = absences.filter((a) => a.subject_id === s.id).reduce((sum, a) => sum + (a.calculated_value || 0), 0);
  ...
  const lastEntry = [...journals].sort((a, b) => toLocalDay(b.date).getTime() - toLocalDay(a.date).getTime())[0];

  const toggle = (t: Task) => updateTask.mutate({ id: t.id, data: { status: t.status === "done" ? "todo" : "done" } });
  const today = new Intl.DateTimeFormat("es-UY", { weekday: "long", day: "numeric", month: "long" }).format(new Date());

  return (
    <div>
      <PageHeader title={`Hola, ${user.name.split(" ")[0]}`} subtitle={today.charAt(0).toUpperCase() + today.slice(1)} />
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6">
        <StatTile
          ...
          href="/tasks"
        />
        <StatTile label="Próxima entrega" value={next ? dueLabel(next.due_date) : "—"} hint={next?.title ?? "Sin entregas"} href="/tasks" />
        <StatTile
          label="Asistencia"
          value={atRisk.length ? `${atRisk.length} en riesgo` : "Al día"}
          hint={atRisk[0] ? `${atRisk[0].subject.name}: ${remainingLabel(atRisk[0].info).toLowerCase()}` : "Ninguna UC cerca del límite"}
          tone={atRisk.some((a) => a.info.status === "danger") ? "danger" : atRisk.length ? "warning" : "success"}
          href="/subjects"
        />
        <StatTile label="Promedio" value={avg ?? "—"} hint={grades.length ? `${grades.length} notas cargadas` : "Sin notas aún"} href="/analytics" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1.4fr_1fr] gap-4 sm:gap-6">
        <Card
          ...
          action={
            <a href="/tasks" className="text-sm text-theme-text-muted no-underline hover:text-theme-text inline-flex items-center gap-1">
        ...
        >
          {upcoming.length === 0 ? (
            ...
            <ul className="list-none m-0 p-0">
              {upcoming.slice(0, 6).map((t) => {
                ...
                return (
                  <li key={t.id} className="flex items-center gap-3 py-2.5 border-b border-theme-border last:border-b-0">
                    <button
                      onClick={() => toggle(t)}
                    ...
                    <a href={`/tasks/${t.slug || t.id}`} className="flex-1 min-w-0 no-underline text-theme-text">
                      <span className="block text-sm font-medium truncate">{t.title}</span>
                      {subjectName(t.subject_id) && <span className="block text-xs text-theme-text-muted truncate">{subjectName(t.subject_id)}</span>}
                    </a>
                    <span
          ...
          {weekEvents.length > 0 && (
            <div className="mt-4 pt-4 border-t border-theme-border">
              <p className="m-0 mb-2 text-xs font-medium uppercase tracking-wide text-theme-text-muted">Calendario (próximos 7 días)</p>
              {weekEvents.map((e) => (
        ...
        <div className="flex flex-col gap-4 sm:gap-6">
          <Card
            ...
            action={
              <a href="/subjects" className="text-sm text-theme-text-muted no-underline hover:text-theme-text inline-flex items-center gap-1">
                UC <ArrowRight size={14} />
              </a>
            }
          >
            {attendance.length === 0 ? (
              <p className="m-0 text-sm text-theme-text-muted">Agregá tus UC para seguir las faltas.</p>
            ) : (
              <ul className="list-none m-0 p-0 flex flex-col gap-3">
                {attendance.slice(0, 5).map(({ subject, info }) => (
                  <li key={subject.id}>
                    <div className="flex justify-between gap-2 text-sm mb-1">
                      <a href={`/subjects/${subject.slug || subject.id}`} className="font-medium no-underline text-theme-text truncate">
                        {subject.name}
                      </a>
                      <span className={`text-xs whitespace-nowrap ${info.status === "danger" ? "text-theme-danger" : info.status === "warning" ? "text-theme-warning" : "text-theme-text-muted"}`}>
                        {remainingLabel(info)}
                      </span>
                    </div>
                    <ProgressBar
          ...
          <Card title="Diario de práctica">
            {lastEntry ? (
              <p className="m-0 mb-4 text-sm text-theme-text-muted line-clamp-3">
                <span className="font-medium text-theme-text">Última: {formatDayLong(lastEntry.date)}.</span> {lastEntry.content}
              </p>
            ) : (
