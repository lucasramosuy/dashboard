...
code="$(curl -s -o /dev/null --max-time 10 -w '%{http_code} %{redirect_url}' "$BASE/")"
...
if [ -n "$asset" ]; then
  check "CSS $asset" "200" "$(curl -s -o /dev/null --max-time 10 -w '%{http_code}' "$ORIGIN$asset")"
else
  echo "FALLA no se encontró el CSS en /login"
  FAIL=1
fi

code="$(curl -s -o /dev/null --max-time 15 -w '%{http_code}' -X POST "$BASE/api/auth/sign-in/email" \
